...
7. **形成报告**：汇总发现的安全问题
8. **降低误报**: 分析总结发现的**漏洞攻击链路**，**确定可利用**防止误报:
   - 如存在Fastjson 反序列化漏洞，进行版本比对和AutoType开关确认
   - 如存在上传漏洞，自动进行上传目标确认， 输出上传目标是OSS还是本地，及外部可访问路径和文件名生成规则
   - 自动梳理确认参数可控性，所有漏洞均需标明验证级别，及绕过验证进行攻击的方式
   - 如上下文中包含测试站点的url信息， 可自行调用浏览器抓取js进行分析，找寻更多漏洞入口或参考信息
   - 无鉴权保护的API接口，需分析业务寻找利用价值
   - 输出完整PoC到文档
...
- 分析完成后请删除临时脚本文件
- 检测Sink和分析，优先 **梳理依赖关系与执行顺序关系** ， **可并行执行提高检测速度**
